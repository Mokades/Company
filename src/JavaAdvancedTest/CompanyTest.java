...
import JavaAdvancedCompany.CompanyService;
import JavaAdvancedCompany.CompanyServiceImpl;
...
@TestMethodOrder(MethodOrderer.OrderAnnotation.class)  // для обозначения порядка тестов
class CompanyTest {
...
CompanyService company = null;
	...
	void testGetSalaryDistribution() {
		...
		assertArrayEquals(expectedDistribution, distribution.toArray(new SalaryIntervalDistribution[] {}));
	}
	...
	private void runListTest(Employee [] expected, List<Employee> list) // Для упрощения кода, чтобы не писать одно и то же
	{
